...
- **Travas** sobre `publicadoRedesEm` (campo novo, gravado pelo orquestrador junto com `status: 'publicado'`, então cobre cron, CLI e /admin) **somado aos carrosséis do bot de mídias** (`registrospublicacao`, origem `bot` — desde 2026-09-26, ver 20-octies): teto `REDES_POSTS_POR_DIA` (padrão 2) nas últimas 24h e intervalo `REDES_INTERVALO_MIN_HORAS` (padrão 4), para a conta inteira. Execução duplicada do cron não vira post extra.
...
- **Vídeos (Reels, Shorts, YouTube longo):** publicados a partir do ARQUIVO gravado e aprovado — ver 20-nonies. **Continua manual:** Stories com enquete (a API da Meta não publica figurinhas interativas).
...
- **Contagem unificada (pedido do dono, 2026-09-26):** a fila e o bot usam a mesma `motivoParaAguardar` (`filaRedes.js`), que soma `Artigo.publicadoRedesEm` e `registrospublicacao` com origem `bot` e resultado `publicado` — teto e intervalo valem para a conta inteira. A fonte é o MongoDB, não a pasta `publicados/` (local, fora do git, invisível para a fila na Vercel).

### 20-nonies. Vídeos: Reels no Instagram e Shorts/longos no YouTube (2026-09-26)

- **Roteiros (bot-gemini):** além dos 2 Shorts (agora com descrição e tags de SEO), o rascunho traz **1 vídeo longo 16:9 (8 a 12 min)**: título ≤ 60 com a palavra-chave no início, descrição de SEO sem link, 8–15 tags, capítulos (1º em 0:00, ≥ 3, ≥ 10 s entre eles) e roteiro em blocos com tempo, fala, B-roll (sem pessoas identificáveis) e texto na tela. Metadados em blocos ```metadados:short-1 / short-2 / longo``` no .md, que o médico pode editar antes de aprovar. Verificação estrutural (`verificarYoutube`) e revisor semântico cobrem os campos novos.
- **O vídeo precisa de aprovação própria:** a aprovação do rascunho cobre o roteiro, não o arquivo gravado. `npm run bot:aprovar -- --slug=<slug> --midia=<arquivo.mp4> --peca=reel-1|reel-2|short-1|short-2|longo` valida o arquivo (`backend/lib/videoRedes.js`, ffprobe) e grava em `meta.aprovacao.midias` o SHA-256, dimensões, duração, responsável e o aviso CFM. O publicador só envia arquivo com esse hash.
- **Especificações conferidas antes de aprovar e de publicar** (limites conservadores — conferir as regras atuais de cada rede): Reel 9:16, 3 s–15 min, ≤ 300 MB; Short vertical ≤ 3 min; longo 16:9 ≥ 1 min; H.264/HEVC + AAC. Considera rotação de vídeo de celular.
- **`npm run bot:publicar -- --slug=<slug> --tipo=reel|short|longo --arquivo=<.mp4> [--peca=...] [--confirmar]`** — prévia por padrão.
  - *Reel:* upload resumível oficial da Meta (container `REELS` + `upload_type=resumable` → bytes para o `uri` em rupload.facebook.com com `Authorization: OAuth`, `offset`, `file_size` → FINISHED → `media_publish`), legenda = a aprovada + identificação + aviso CFM. Conta na cadência unificada da conta (20-ter): a fila soma só posts do bot com `redes.instagram`.
  - *Short/longo:* YouTube Data API v3, upload resumível (`backend/lib/youtube.js`), categoria Educação, pt-BR. Descrição = a aprovada + link do artigo + capítulos (longo) + aviso educativo + CVV 188/SAMU 192 (tema sensível) + identificação completa + aviso CFM 2.454/2026 + `#Shorts` (Shorts). YouTube não entra na cota do Instagram.
- **OAuth do YouTube:** `YOUTUBE_CLIENT_ID`/`YOUTUBE_CLIENT_SECRET` de um cliente OAuth "App para computador" (Google Cloud, YouTube Data API v3 ativada); `npm run youtube:autorizar` abre o consentimento (escopo `youtube.upload`, conta dona do canal) e grava `YOUTUBE_REFRESH_TOKEN` direto no `.env`, sem exibir. **Service Account não serve** (o YouTube não deixa conta de serviço publicar em canal). **Projeto não auditado:** o YouTube força uploads para "private" até a auditoria de conformidade da API — o publicador avisa quando a privacidade aplicada difere da pedida (`YOUTUBE_PRIVACIDADE`, padrão `public`). Cota: cada upload custa 1.600 das 10.000 unidades/dia padrão.
- **Logs:** `meta.publicacao.videos[<peça>]` (rede, id, link/permalink, status HTTP, SHA-256, privacidade) + `meta.publicacao.logs` + `registrospublicacao` (origem `bot`). Nunca publica a mesma peça duas vezes.
- **Vídeo do carrossel aprovado** (`npm run bot:video-carrossel -- --slug=<slug> --narrar | --audio="<faixa.mp3>" [--saida=...]`, `scripts/bot-video-carrossel.js`): desenha os slides do .md aprovado (mesmo desenho do carrossel) e monta um MP4 1080×1920 (H.264 + AAC, 30 fps), cada slide 4:5 centralizado com faixas **na cor do próprio slide** (`#0d3330` escuro, `#faf7f2` claro — sem moldura). `--narrar`: cada slide dura a sua narração (Azure, `pt-BR-AntonioNeural`, a voz do site) + 0,5 s; abre com "Narração em voz sintética."; conta na trava mensal da cota do Azure (`backend/lib/azureTts.js`, mesmo registro `narracoes-geradas/uso-azure-AAAA-MM.json` da narração dos artigos). `--audio`: faixa externa **com licença de uso comercial** (a API não dá acesso à biblioteca de músicas do Instagram), dividida igualmente entre os slides. Saída padrão `<pasta do aprovado>/<slug>-videos/reel-carrossel.mp4`; depois segue o fluxo normal: `bot:aprovar --midia --peca=reel-carrossel` e `bot:publicar --tipo=reel --peca=reel-carrossel`. Adaptado de um rascunho do dono: voz do site em vez de Francisca, texto escapado no SSML, nunca narra "Slide N", cores da paleta em vez de `#0f172a`.
- **Não validado ainda com vídeo real** (2026-09-26): upload de Reel e de YouTube foram implementados pelas documentações oficiais e testados até as travas, sem envio — a primeira publicação real de cada tipo deve ser acompanhada.
