...
- **`backend/lib/llm.js`:** `gerarJson({sistema, usuario, schema})` — Gemini (`@google/genai`, modelo `GEMINI_MODEL`, padrão `gemini-flash-latest`) com saída em JSON Schema; reserva Groq (`groq-sdk`, `GROQ_MODEL`, padrão `llama-3.3-70b-versatile`, modo JSON). Repete em 429/5xx. Chaves `GEMINI_API_KEY`/`GROQ_API_KEY` só no `.env` local (mascaradas pelo verificador de segredos por serem `*_KEY`).
- **`npm run bot:gerar-posts -- --slug=<slug>` ou `--proximos=N`** (`scripts/bot-gemini.js`, alias `bot:gemini`): rascunho do pacote Essencial (ganchos, carrossel, legenda, 2 Reels, Stories, 2 LinkedIn, YouTube) a partir do texto limpo do artigo, com as regras de fidelidade/CFM/idioma/escrita no prompt de sistema. **Verificação automática** anota alertas no topo: número que não está no artigo, termo do CFM, "Antonio", `#psiquiatria`, inglês nas artes, limites de tamanho, CVV em tema sensível, narrativa composta em relato. Identificação do médico é acrescentada pelo código, nunca pelo LLM. Saída em `CONTEUDO_INSTAGRAM/rascunhos/` (**fora do git**). Não publica e não mexe na fila (Regra 5).
